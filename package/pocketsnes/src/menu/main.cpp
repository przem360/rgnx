...
u16 IntermediateScreen[SNES_WIDTH * SNES_HEIGHT_EXTENDED*2];
SDL_Surface *image;
...
{
	//fprintf(stderr, "S9xInitUpdate: GFX.Pitch=%d GFX.RealPitch=%d GFX.Screen=%p IntermediateScreen=%p\n", GFX.Pitch, GFX.RealPitch, GFX.Screen, IntermediateScreen);
    //fflush(stderr);
...
bool8_32 S9xDeinitUpdate (int Width, int Height, bool8_32)
{
	...
	switch (0)
	{
		...
		case 4: /* Crop scaling */
		{
			...
			u8 *dst = (u8*) sal_VideoGetBuffer(); // reset
            /*
            ...
            */
			//fprintf(stderr, "PAL=%d h=%d SNES_HEIGHT=%d SNES_HEIGHT_EXTENDED=%d sizeof IntermediateScreen=%zu\n", PAL, h, SNES_HEIGHT, SNES_HEIGHT_EXTENDED, sizeof(IntermediateScreen));
			//fflush(stderr);
			for (y = 0; y < h; y++)
			{
				memmove(dst, src, 256 * sizeof(u16));
	...
	//fprintf(stderr, "About to flip, mScreen=%p pixels=%p w=%d h=%d pitch=%d\n",
    //mScreen, mScreen->pixels, mScreen->w, mScreen->h, mScreen->pitch);
	//fflush(stderr);
...
int SnesInit()
{
	...
	// One contiguous block: Screen + SubScreen together
static u16 ScreenBuffer[SNES_WIDTH * SNES_HEIGHT_EXTENDED * 2];  // doubled for SubScreen

GFX.Screen    = (u8*) IntermediateScreen;
GFX.SubScreen = (u8*) IntermediateScreen + (SNES_WIDTH * SNES_HEIGHT_EXTENDED * sizeof(u16));
...
GFX.ZPitch = GFX.Pitch >> 1;
	...
	//fprintf(stderr, "PRE-INIT: Screen=%p SubScreen=%p Delta=%d diff=%ld\n",
    //GFX.Screen, GFX.SubScreen,
    //GFX.Delta,
    //(long)(GFX.SubScreen - GFX.Screen));
	//fflush(stderr);
