...
s32 FileSelect()
{
	...
	while (menuExit == 0) {
		...
		if (strcmp(mRomList[focus].displayName, previousRom) != 0) {
			char dummy[SAL_MAX_PATH], fileNameNoExt[SAL_MAX_PATH], extDummy[SAL_MAX_PATH];
			sal_DirectorySplitFilename(mRomList[focus].filename, dummy, fileNameNoExt, extDummy);
			snprintf(previewPath, SAL_MAX_PATH, "%s/previews/%s.%s", sal_DirectoryGetHome(), fileNameNoExt, "png");
