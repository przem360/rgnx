...
void sal_DirectorySplitFilename(const char *wholeFilename, s8* path, s8 *filename, s8 *ext)
{
	...
	if (dot>=0)
	{
		int fnStart = (slash >= 0) ? slash+1 : 0;
		int fnLen = dot - fnStart;
		if (fnLen < 0) fnLen = 0;
		if (fnLen >= SAL_MAX_PATH) fnLen = SAL_MAX_PATH-1;
		memmove(filename, wholeFilename+fnStart, fnLen);
		filename[fnLen] = 0;
		int extLen = len - dot - 1;
		if (extLen < 0) extLen = 0;
		if (extLen >= SAL_MAX_PATH) extLen = SAL_MAX_PATH-1;
		memmove(ext, wholeFilename+dot+1, extLen);
		ext[extLen] = 0;
	}
	else
	{
		int fnStart = (slash >= 0) ? slash+1 : 0;
		int fnLen = len - fnStart;
		if (fnLen < 0) fnLen = 0;
		if (fnLen >= SAL_MAX_PATH) fnLen = SAL_MAX_PATH-1;
		memmove(filename, wholeFilename+fnStart, fnLen);
		filename[fnLen] = 0;
